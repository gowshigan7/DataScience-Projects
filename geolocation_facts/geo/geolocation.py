...
geolocation.py
==============
Feature : Résolution d'adresse en coordonnées GPS (geocoding) et calcul de distance.
...
def resolve_location(location: str, api_key: str = "") -> tuple[float, float]:
    ...
    Args:
        ...
        api_key (str): Clé API Google Geocoding (optionnel). Si vide, utilise Nominatim.
    ...
    effective_key = api_key or GOOGLE_GEOCODING_API_KEY
    if effective_key:
...
def calculate_distance(lat1: float, lng1: float, lat2: float, lng2: float) -> float:
    ...
    """
    r = EARTH_RADIUS_KM * METERS_PER_KM  # Rayon de la Terre en mètres
